    ...
    return (
      <>
        ...
        <PageShell>
          ...
          <input type="text" placeholder="e.g. Kazim" value={name}
            onChange={e => setName(e.target.value)} style={inputStyle} autoFocus />

          <FieldLabel>Room code</FieldLabel>
          <input type="text" placeholder="e.g. XK7M2P"
            value={code}
            onChange={e => setCode(e.target.value.toUpperCase())}
            onKeyDown={e => e.key === 'Enter' && handleJoin()}
            maxLength={6}
            style={{ ...inputStyle, letterSpacing: '0.2em', textTransform: 'uppercase', fontSize: 20, textAlign: 'center' }}
          />
    ...
    <>
      ...
      <PageShell>
        ...
        <div style={{ width: '100%', textAlign: 'left', marginBottom: 20 }}>
          <div style={{ fontSize: 11, fontFamily: 'monospace', letterSpacing: '0.1em', color: '#8a8880', marginBottom: 12, textTransform: 'uppercase' }}>Players in room</div>
          <PlayerRow name="Alex" isHost />
          <PlayerRow name={name} />
          <div style={{ fontSize: 13, color: '#8a8880', fontStyle: 'italic', marginTop: 8, textAlign: 'center' }}>Waiting for more players...</div>
        </div>
...
function Sub({ children }: { children: React.ReactNode }) {
  return <p style={{ fontSize: 14, color: '#7a7870', lineHeight: 1.7, marginBottom: 24 }}>{children}</p>;
}

function FieldLabel({ children }: { children: React.ReactNode }) {
  return <div style={{ fontSize: 12, fontFamily: 'monospace', color: '#00274C', letterSpacing: '0.06em', textAlign: 'left', marginBottom: 6, textTransform: 'uppercase', fontWeight: 700 }}>{children}</div>;
}
